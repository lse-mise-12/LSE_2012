...
int temp_driver_open(struct inode *inode, struct file *filp);
int temp_driver_release(struct inode *inode, struct file *filep);
ssize_t temp_driver_read(struct file *filep, char *buf, size_t count, loff_t *f_pos);
ssize_t temp_driver_write(struct file *filep, const char *buf, size_t count, loff_t *f_pos);
...
/* Global variables */
int temp_driver_major = 80;


int temp_driver_init (void){
	int result;

	//Register type of device: character (or block, net)

	result = register_chrdev(temp_driver_major, "temp_reg", &temp_fops);
	if (result < 0){
	...
	printk("<1>Temperature driver loaded\n");
	return result;
}

void temp_driver_exit (void) {
	//free major number
	unregister_chrdev(temp_driver_major, "temp_reg");
	//Unlock access to register
	//release_region(REG_ADDR_PEDR, REG_NUMB);
	printk("<1>Temperature driver unloaded\n");
}
